...
Report bugs at https://github.com/PhilippRisius/nbgrader-jupyterquiz/issues.
...
nbgrader-jupyterquiz could always use more documentation, whether as part of the official nbgrader-jupyterquiz docs, in docstrings, or even on the web in blog posts, articles, and such.

Submit Feedback
~~~~~~~~~~~~~~~

The best way to send feedback is to file an issue at https://github.com/PhilippRisius/nbgrader-jupyterquiz/issues.
...
Ready to contribute? Here's how to set up ``nbgrader-jupyterquiz`` for local development.

#. First, clone the ``nbgrader-jupyterquiz`` repo locally.

    * If you are not a ``nbgrader-jupyterquiz`` collaborator, first fork the ``nbgrader-jupyterquiz`` repo on GitHub, then clone your fork locally.

        .. code-block:: console

            git clone git@github.com:your_name_here/nbgrader-jupyterquiz.git

    * If you are a ``nbgrader-jupyterquiz`` collaborator, clone the ``nbgrader-jupyterquiz`` repo directly.
...
#. Install your local copy into a development environment. Using ``virtualenv`` (``virtualenvwrapper``), you can create a new development environment with:
    ...
    .. code-block:: console
        ...
        mkvirtualenv nbgrader_jupyterquiz
        cd nbgrader_jupyterquiz/
        make dev
    ...
    This installs ``nbgrader_jupyterquiz`` in an "editable" state, meaning that changes to the code are immediately seen by the environment. To ensure a consistent coding style, `make dev` also installs the ``pre-commit`` hooks to your local clone.
...
#. Before merging, your Pull Request will need to be based on the `main` branch of the ``nbgrader-jupyterquiz`` repository. If your branch is not up-to-date with the `main` branch, you can perform similar steps as above to update your branch:
...
#. Once your Pull Request has been accepted and merged to the `main` branch, several automated workflows will be triggered:
    ...
    - `ReadTheDocs` will automatically build the documentation and publish it to the `latest` branch of `nbgrader_jupyterquiz` documentation website.
    - If your branch is not a fork (i.e. you are a maintainer), your branch will be automatically deleted.

You will have contributed to ``nbgrader_jupyterquiz``!
...
#. The pull request should include tests and should aim to provide `code coverage <https://en.wikipedia.org/wiki/Code_coverage>`_ for all new lines of code. You can use the `--cov-report html --cov nbgrader_jupyterquiz` flags during the call to ``pytest`` to generate an HTML report and analyse the current test coverage.
...
.. code-block:: console
    ...
    python -m pytest tests/test_nbgrader_jupyterquiz.py

You can also directly call a specific test class or test function using:

.. code-block:: console

    python -m pytest tests/test_nbgrader_jupyterquiz.py::TestClassName::test_function_name
...
.. code-block:: console
    ...
    validate-docstrings src/nbgrader_jupyterquiz/**.py
...
Please note that this project is released with a `Contributor Code of Conduct <https://github.com/PhilippRisius/nbgrader_jupyterquiz/blob/main/CODE_OF_CONDUCT.md>`_.
